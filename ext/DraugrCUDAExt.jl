...
"""
    amg_resetup!(hierarchy, A_new::CuSparseMatrixCSR, config; partial=true, update_P=false)

AMG resetup accepting a CUDA sparse CSR matrix. Keeps the CSR arrays on the
device and forwards to the main `CSRMatrix`-based resetup.
"""
function Draugr.amg_resetup!(hierarchy::AMGHierarchy{Tv, Ti},
    ...
    A_csr = Draugr.csr_from_gpu(A_new)
