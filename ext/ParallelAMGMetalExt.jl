...
function ParallelAMG.amg_resetup!(hierarchy::AMGHierarchy{Tv, Ti},
    ...
    if nlevels == 0
        ParallelAMG._update_coarse_solver!(hierarchy, A_new; backend=backend, block_size=block_size)
        return hierarchy
    end
    # Copy nonzero values from new matrix into existing level 1
    level1 = hierarchy.levels[1]
    ParallelAMG._copy_nzvals!(level1.A, A_new; backend=backend, block_size=block_size)
